...
class C_perhitungan_prom extends CI_Controller
{
    ...
    {
        $data['content'] = "user/perhitungan/V_perbandingan_prom"; //admin merupakan folder didalam view, v_data_kedelai merupakan file php yg merupakan isi dari halam
    ...
    public function hasil_prom()
    {
        ...
        if (count($alt) > 1) { //jika kedelai yang dibandingkan lebih dari 1
            ...
            $data['content'] = "user/perhitungan/V_perhitungan_prom"; //admin merupakan folder didalam view, v_data_kedelai merupakan file php yg merupakan isi dari halam
            $this->load->view('user/main', $data); //memanggil file main.php yang berada didalam folder admin pada bagian view, sekaligus menyisipkan $data pada file tsb
        } else { // jika hanya memilih 1 kedelai
            $this->session->set_flashdata('warning', 'Harap Pilih 2 atau lebih data kedelai!');
            redirect(base_url('index.php/user/c_perhitungan_prom'));
        }
    }
